...
Lets_test <- function(se, dge, n_SV, MDS_folder, MDS_name){
...
# MDS_name: should be folder/xxsamples_seed_xx
    ...
    if(n_SV==0){
        ...
        # MDS logcpm
        col <- paste0(se_sv$group, "_", se_sv$Sex)
        col <- c(A_Female = "purple3", A_Male = "cornflowerblue", B_Female = "orange2", B_Male = "firebrick4")[col]
        pdf(paste0(MDS_name, "_logcpm.pdf"), width = 8, height = 6)
        limma::plotMDS(assay(se_sv, "logcpm"), labels = paste0(se_sv$group, "_", se_sv$Sex), col = col)
        dev.off()
        test_res_list[["MDSdata"]] <- plotMDS(assay(se_sv, "logcpm"), labels = paste0(se_sv$group, "_", se_sv$Sex), col = col, plot = FALSE)
        ...
        limma::plotMDS(assay(se_sv, "corrected"), labels = paste0(se_sv$group, "_", se_sv$Sex), col = col, main = paste("MDS: ", basename(MDS_name)," - n.sv =", n_SV, ", corrected value"))
        dev.off()
        test_res_list[["MDSdata"]] <- plotMDS(assay(se_sv, "corrected"), labels = paste0(se_sv$group, "_", se_sv$Sex), col = col, plot = FALSE)
...
subsampling <- function(se, sampleSize, seed){
    ...
    target <- c(Male = sampleSize/3, Female = sampleSize*2/3) 
    set.seed(seed)
    idx_sub <- unlist(lapply(split(seq_len(ncol(se)), se$Sex), \(x) {
    Sex_name <- as.character(unique(se$Sex[x]))
    sample(x, target[Sex_name])
    }))
    se_sub <- se[, idx_sub]

    # randomly assign them into two groups (but make sure two batches are balanced)
    assignment <- dplyr::bind_rows(lapply(split(seq_len(ncol(se_sub)), se_sub$Sex), \(x){
        set.seed(seed)
