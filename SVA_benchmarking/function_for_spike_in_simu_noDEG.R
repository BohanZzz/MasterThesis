...
# start to simulate the effect size of each coefficient
    bco2 <- bco1 <- rep(0, n)  # initialize them to 0
...
# =============== 1. Align genes between a and beta ==================
    a <- assay(se, "counts")
    beta <- cbind(batch1=rowData(se)$batch1, batch2=rowData(se)$batch2)
...
# =============== 4. Compute mu (predicted mean expression) ==========
    predict_MM <- model.matrix(~ batch1 + batch2, data = colData(se))
